...
class Child extends Parent {
    int extraValue; // 子类新增的 int 成员
    
    public Child(int base, int extra) {
        super(base);          // 调用父类构造，初始化 baseValue
        this.extraValue = extra; // 初始化子类自己的成员
    }
}
```
...
- 创建子类对象时，你需要传入**全部参数**（包括父类需要的和子类新增的）。
    
- 向上转型：将子类对象赋值给父类引用时，**只会丢失子类特有的部分**（无法通过父类引用直接访问），但对象本身依然是完整的子类实例。
    
- 向下转型：只要该对象确实是子类类型，就可以强制转回子类引用，从而访问所有成员（包括新增的属性/方法）。
...
方法可以被重写。子类和父类一模一样的方法，子类的方法会覆盖父类的方法。
变量不会被重写，都可以读到
